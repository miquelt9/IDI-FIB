...
void MyGLWidget::viewTransform()
{
  ...
  glm::mat4 View (1.0f);
  View = glm::translate(View, glm::vec3(0,-altura,-dist));
...
void MyGLWidget::ini_camera()
{
  ...
  OBS = glm::vec3(0, altura, dist);//glm::vec3(center[0], center[1], center[2]+dist);
...
void MyGLWidget::modelTransformPatricio1()
{
  ...
  transform = glm::scale(transform, glm::vec3(escala*0.25));  // Per aquest exercici volem que faci 1 altura
  transform = glm::rotate(transform, rotacio + float(M_PI/2.0), glm::vec3(0,1,0));    //rotacio que apliquem amb R

  transform = glm::translate(transform, glm::vec3(0,2,0));        // L'aixequem pk no quedi solapat amb el terra
  transform = glm::scale(transform, glm::vec3(4/(max[1]-min[1]))); //Volem que faci 4 d'altura
  transform = glm::translate(transform, -center);       // el centrem
...
void MyGLWidget::modelTransformPatricio2()
{
  ...
  transform = glm::scale(transform, glm::vec3(escala*0.25));
...
void MyGLWidget::modelTransformPatricio3()
{
  ...
  transform = glm::translate(transform, glm::vec3(-2,0,-2));
  transform = glm::scale(transform, glm::vec3(escala*0.25));
  transform = glm::rotate(transform, rotacio + float(M_PI), glm::vec3(0,1,0));
...
void MyGLWidget::keyPressEvent(QKeyEvent* event)
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_R: { // rotem
      rotacio += (float)M_PI/8;
      break;
    }
    case Qt::Key_O: { // canvi optiva 0->prespectiva, 1-> orto
      optica = (optica+1)%2;
      projectTransform();
      break;
    }
    case Qt::Key_Z: { //zoom_in
      FOV -= 0.1; //prespectiva
      left = bottom = -(glm::sin(FOV/2)*dist);  //orto
      top = right = (glm::sin(FOV/2)*dist);
      projectTransform();
      break;
    }
    case Qt::Key_X: { //zoom_out
      FOV += 0.1;
      left = bottom = -(glm::sin(FOV/2)*dist);  //orto
      top = right = (glm::sin(FOV/2)*dist);
      projectTransform();
...
void MyGLWidget::mouseMoveEvent(QMouseEvent* event)
{
  ...
  psi += 0.02 * (x_new - x_click);
  theta += 0.02 * (y_new - y_click);

  // std::cout << x_new << " " << y_new << std::endl;
  // std::cout << x_click << " " << y_click << std::endl;
